...
def tmm_bool_expressions(expression, Lt, Lf, tac):
    ...
    if isinstance(expression, Bool):
        ...
        deref = DeRef(expression.element, None, None)
        if isinstance(expression.element.type, str):
            expr_el_ty = scopes[0][expression.element.type][0]
        else:
            expr_el_ty = expression.element.type

        deref.type = expr_el_ty.type

        dot = Dot(deref, expression.var, None, None)
        dot.type = expression.var.type

        tmm_bool_expressions(dot, Lt, Lf, tac)
        ...
        for i, arg in enumerate(expression.args):
            ...
            op, args = tmm_expressions(arg, tac)
            tac["body"].append(
...
def tmm_expressions(expression, tac):
    ...
    if isinstance(expression, Call):
        ...
        return evaluate_bool_expr(expression, tac)
        ...
        return tmm_expressions(dot, tac)'''
...
def tmm_statements(instruction, tac):
    ...
    if isinstance(instruction, Block):
        ...
        if isinstance(instruction.target, Variable):
            ...
            '''deref = DeRef(instruction.target.element, None, None)
            if isinstance(instruction.target.element.type, str):
                el_type = scopes[0][instruction.target.element.type][0]
            else:
                el_type = instruction.target.element.type

            deref.type = el_type.type

            dot = Dot(deref, instruction.target.var, None, None)
            dot.type = instruction.target.var.type

            assign = Assign(dot, instruction.expr,
                            instruction.lineno, instruction.col)
            tmm_statements(assign, tac)'''
            temp = '%' + str(next_temorary)
            next_temorary += 1
