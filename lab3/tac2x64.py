...
class Generator_x64:
    def __init__(self):
        ...
        self.strs = []
        self.stack_pos = 0
        self.binop = {'add' : 'addq', 'sub' : 'subq', 'mul' : 'mulq', 'and' : 'andq', 'or' : 'orq', 'xor' : 'xorq'}
        self.unop = {'not' : 'notq', 'neg' : 'negq'}
        self.shiftop = {'shl' : 'salq', 'shr' : 'sarq'}
        self.div = {'div' : '%rad', 'mod' : '%rdx'}
        self.condition = ['je', 'jz', 'jne', 'jnz', 'jl', 'jnge', 'jle', 'jng', 'jg', 'jnle', 'jge', 'jnl']
    ...
    def to_str(self, tac_expr):
        ...
        if (op == 'const'):
            ...
            self.strs += [f"\tmovabsq ${arg1}, %rax", f"\tmovq %rax, {-8 * pos}(%rbp)"]
        elif (op == 'copy'):
            pos = self.get_pos(dest)
            self.strs += [f"\tmovq {-8 * self.get_pos(arg1)}(%rbp), %r8", f"\tmovq %r8, ${-8 * pos}(%rbp)"]
        elif (op == 'print'):
            self.strs += [f"\tmovq {-8 * self.get_pos(arg1)}(%rbp), %rdi", "\tcallq bx_print_int"]
        elif op == 'label':
            self.strs += [arg1 + ":"]
        elif op == 'jmp':
            self.strs += [f"\tjmp {arg1}"]
        elif op in self.binop.keys():
            op_name = self.binop[op]
            self.strs += [f"\tmovq {-8 * self.get_pos(arg1)}(%rbp), %r8", 
                            f"\tmovq {-8 * self.get_pos(arg2)}(%rbp), %r9", 
                            f"\t{op_name} %r8, %r9",
                            f"\tmovq %r9, {-8 * self.get_pos(dest)}(%rbp)"]
    ...
    def main(self, tac_file):
        for line in tac_file:
            self.to_str(line)
        head = ['\t.global main', '\t.text', 'main', '\tpushq %rbp', 
                '\tmoveq %rsp, %rbp', f'\tsubq ${8 * len(self.stack)}, %rsp']
        tail = ['\tmovq %rbp, %rsp ', '\tpopq %rbp ', '\tmovq $0, %rax', '\tretq']
