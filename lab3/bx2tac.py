...
def bmm_expressions(expression, tac):
    ...
    if isinstance(expression, Number):
        ...
        tac[0]['body'].append(
            {'opcode': 'const', 'args': [expression.value], 'result': result})
        return result
        ...
        next_temorary += 1
        tac[0]["body"].append(
            {"opcode": operation_covert[expression.op], "args": [arg], "result": result})
        ...
        result = '%' + str(next_temorary)
        next_temorary += 1
        tac[0]["body"].append({"opcode": operation_covert[expression.op], "args": [
                              arg_left, arg_right], "result": result})
...
def bmm_statements(instructions, tac):
    ...
    for instruction in instructions:
        if isinstance(instruction, VarDec):
            ...
            tac[0]["body"].append(
                {"opcode": "copy", "args": [arg], "result": result})

        elif isinstance(instruction, Assign):
            arg = bmm_expressions(instruction.expr, tac)
            tac[0]["body"].append({"opcode": "copy", "args": [
                                  arg], "result": variables[instruction.target.name]})

        elif isinstance(instruction, Call):
            arg = bmm_expressions(instruction.args[0], tac)
            tac[0]["body"].append(
                {"opcode": instruction.function, "args": [arg], "result": None})
