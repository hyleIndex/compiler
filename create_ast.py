...
class Pointer(Type):
    ...
    def __eq__(self, other):
        ...
        if isinstance(self.type, VOID) or isinstance(other.type, VOID):
            return True
...
class Array(Type):
    ...
    def sizeof(self):
        return self.length.value * self.type.sizeof()
...
class TypeDecl:
    ...
    def __eq__(self, other):
        return isinstance(other, Type) and other == self.type
...
class VarDec(Statment):
    ...
    def __str__(self):
        return f'{str(self.name)} = {str(self.initial)}'
...
class Assign(Statment):
    ...
    def __str__(self):
        return f'{str(self.target)} = {str(self.expr)}'
...
class Dot(Expr):
    ...
    def __str__(self):
        return f'{str(self.element)}.{self.var}'
